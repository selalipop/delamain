...
import { startRecording } from "./audioGenerator";
...
const ImageDescription: React.FC = () => {
  ...
  const transcribeTask = new Promise(async () => {
    ...
    if (session.isOk) {
      ...
      while (true) {
        const audioData = await audioGenerator.next();
        const value = audioData.value;
        if (!value) {
          console.log("audio data is null");
          continue;
        }
        console.log("audio data is not null", value);
