...
const ImageDescription: React.FC = () => {
  ...
  useEffect(() => {
    ...
    s3Client.send(new PutBucketCorsCommand(params)).then((data) => {
  ...
  const vad = useMicVAD({
    onSpeechEnd: async (audio) => {
      const blob = new Blob([audio], { type: "wav" });
      const params = {
        Bucket: BUCKET_NAME,
    ...
    document.getElementById("result").innerText = `Audio URL: ${result.Location}`;
